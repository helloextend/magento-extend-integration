...
class CatalogProductSaveEntityAfterTest extends TestCase
{
  ...
  protected function setUp(): void
  {
    ...
    $this->event = $this->getMockBuilder(Event::class)
      ...
      ->disableOriginalConstructor()
      ->getMock();
    $this->event->expects($this->any())->method('getProduct')->willReturn($this->productMock);
    ...
    $this->import = $this->objectManager->getObject(
      CatalogProductSaveEntityAfter::class,
      [
        'logger' => $this->logger,
  ...
  public function testExecutesProductsObserverAndSendsProductToCreateEndpointWhenProductIsNew() {
    $this->productMock->expects($this->any())->method('isObjectNew')->willReturn(true);
    $this->productObserverHandler->expects($this->once())
        ->method('execute')
        ->with(
          $this->equalTo(['path' => Integration::EXTEND_INTEGRATION_ENDPOINTS['webhooks_products_create'], 'type' => 'middleware']),
  ...
  public function testExecutesProductsObserverAndSendsProductToCreateEndpointWhenProductIsNotNew() {
    $this->productMock->expects($this->any())->method('isObjectNew')->willReturn(false);
    $this->productObserverHandler->expects($this->once())
        ->method('execute')
        ->with(
          $this->equalTo(['path' => Integration::EXTEND_INTEGRATION_ENDPOINTS['webhooks_products_update'], 'type' => 'middleware']),
          $this->equalTo($this->productMock),
          []
        );
    $this->import->execute($this->observer);
