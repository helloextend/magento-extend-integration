...
class ShipmentObserverHandlerTest extends TestCase
{
  ...
  protected function setUp(): void
  {
    ...
    $this->metadataMock = [
      ...
      'magento_store_uuids' => [
        $this->magentoStoreIdMock
      ]
    ];
    $this->metadataBuilder->expects($this->any())->method('execute')->willReturn($this->metadataMock);
    ...
    $this->order->expects($this->any())->method('getStoreId')
    ...
    $this->shipment = $this->getMockBuilder(Shipment::class)
      ...
      ->disableOriginalConstructor()
      ->getMockForAbstractClass();
    $this->shipment->expects($this->any())->method('getId')
      ->willReturn($this->shipmentIdMock);
    $this->shipment->expects($this->any())->method('getOrder')
