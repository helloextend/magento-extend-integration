...
class AccessTokenBuilder
{
  ...
  {
    $integrationId = (int)$this->scopeConfig->getValue(\Extend\Integration\Service\Api\Integration::INTEGRATION_ENVIRONMENT_CONFIG);
    $storeIds = $this->storeIntegrationRepository->getListByIntegration($integrationId);

    if (count($storeIds) > 0) {
      $storeId = $storeIds[0];
      $integration = $this->storeIntegrationRepository->getByStoreIdAndIntegrationId($storeId, $integrationId);
